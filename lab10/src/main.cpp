...
public:
    ...
    void init(const std::string &resourceDirectory)
    {
        ...
        meshProg->addUniform("lightColor");
    ...
    void initGeom(const std::string &resourceDirectory)
    {
        ...
        eeveeModel = loadMultModel("/eevee.obj", resourceDirectory);
    }
    ...
    {
        glUniform3f(curS->getUniform("lightPos"), -2.0f, 2.0f, 2.0f);
    ...
    void render()
    {
        ...
        M->pushMatrix();
        M->loadIdentity();
        std::pair<glm::vec3, glm::vec3> eeveeBounds = ModelUtils::measureModel(eeveeModel);
        M->multMatrix(ModelUtils::getNormalizationMatrix(eeveeBounds.first, eeveeBounds.second));
        M->translate(vec3(0, -20.0f, 0));
        M->rotate(glm::radians(-90.0f), vec3(1, 0, 0));
        SetLight(meshProg);
        SetMaterial(meshProg, 0);
        glUniformMatrix4fv(meshProg->getUniform("M"), 1, GL_FALSE, value_ptr(M->topMatrix()));
        for (auto &shape : eeveeModel)
        {
            shape->draw(meshProg);
        }
        M->popMatrix();
