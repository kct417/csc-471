...
void Particle::rebirth(float t, vec3 start)
{
    ...
    float r = 50.0f * pow(randFloat(0.0f, 1.0f), 1.0f / 3.0f);

    x.x = start.x + r * sin(phi) * cos(theta);
    x.y = start.y + r * sin(phi) * sin(theta) / 2;
